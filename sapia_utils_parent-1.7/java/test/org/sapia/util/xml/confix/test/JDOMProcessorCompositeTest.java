...
import org.sapia.util.xml.confix.JDOMProcessor;
 ...
 * </dl>
 */
public class JDOMProcessorCompositeTest extends TestCase {
  ...
  private static final String XML_JDOM_HANDLER =
	...
	private static final String XML_CONSUMER =
		"<WrappedTextualConfig name=\"config7\">" +
		"    <NamedValue name=\"param1\" value=\"value1\"/>" +
		"    <XMLConsumerConfig type=\"ant\">" +
		"        <build default=\"build\"><target name=\"build\"><javac/></target></build>" +
		"    </XMLConsumerConfig>" +
		"    THIS IS A FINAL CONFIG CONTENT" +
		"</WrappedTextualConfig>";    
  ...
  private JDOMProcessor _theProcessor;

  public static void main(String[] args) {
    TestRunner.run(JDOMProcessorCompositeTest.class);
  }

  public JDOMProcessorCompositeTest(String aName) {
    super(aName);
  }
  ...
  public void setUp() {
    ...
    _theProcessor = new JDOMProcessor(anObjectFactory);
  }
  ...
  public void testJdomHandler() throws Exception {
	...
	public void testXMLConsumer() throws Exception {
		ByteArrayInputStream anInput =
						new ByteArrayInputStream(XML_CONSUMER.getBytes());
		Object anObject = _theProcessor.process(anInput);
		assertTrue("The returned object is not a TextualConfig", anObject instanceof WrappedTextualConfig);
		...
		XMLConsumerConfig aCustomConfig = (XMLConsumerConfig) someCustomCongigs.next();
		assertEquals("The type of the custom config is invalid", "ant", aCustomConfig.getType());
