...
public class HeartbeatScenariosTest extends EventChannelControllerTestSupport {
  ...
  public void testHeartbeat() {
    ...
    master.getController().checkStatus();
    assertEquals("Heartbeat request was not received by slave1", clock.currentTimeMillis(), slave1.getController().getContext()
        .getLastHeartbeatRequestReceivedTime());

    assertEquals("Heartbeat request was not received by slave2", clock.currentTimeMillis(), slave2.getController().getContext()
        .getLastHeartbeatRequestReceivedTime());
  }
  ...
  public void testDownDetected() {
    ...
    assertFalse("Down notification not received by slave1", slave1.containsSibling(slave2.getNode()));
    assertFalse("Down node not removed from master", master.containsSibling(slave2.getNode()));
