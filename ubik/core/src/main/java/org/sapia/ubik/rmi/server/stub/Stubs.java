...
public class Stubs {
   ...
   * 
   * @param object
   *          an {@link Object} that is expected to be a {@link Stub}.
   * @return the {@link StubInvocationHandler} that the stub/dynamic proxy
   *         wraps.
   */
   ...
   * 
   * @param name
   *          the JNDI {@link Name} of the object to which the remote reference
   *          corresponds.
   * @param domain
   *          the domain in the context of which the intended remote reference
   *          will exist.
   * @param handler
   *          the {@link StubInvocationHandler} to convert.
   * @return a new {@link RemoteRefStateless} instance.
   */
  public static RemoteRefStateless convertToStatelessRemoteRef(Name name, String domain, StubInvocationHandler handler) {
  ...
  /**
   * Tries sending the given command to the endpoint represented by the passed
   * in {@link RemoteRefContext}:
   * 
   * <ol>
   * <li>Acquires a {@link Connection} from the given context's connection pool
   * (see {@link RemoteRefContext#getConnections()}.
   * <li>Tries sending the given command using the connection.
   * <li>If a {@link RemoteException} is caught, clears the pool (see
   * {@link Connections#clear()} and tries another time.
   * <li>If an exception other than {@link RemoteException} is caught, then that
   * exception is thrown.
   * <li>If sending the command succeeds, this method returns that command's
   * return value - and also releases to its pool the connection that was used.
   * <li>If a {@link RemoteException} is thrown upon the second attempt, then it
   * is rethrown to the caller.
   * </ol>
   * 
   * @param command
   *          the {@link RMICommand} to send.
   * @param target
   *          the {@link RemoteRefContext} corresponding to the endpoint to
