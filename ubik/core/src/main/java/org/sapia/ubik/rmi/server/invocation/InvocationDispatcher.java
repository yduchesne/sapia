...
public class InvocationDispatcher implements Module {
  ...
  public static class InvocationStats {
    Stopwatch acquireCon = Stats.createStopwatch(InvocationDispatcher.class, "AcquireConnection", "Time to acquire a connection");

    Stopwatch invokeSend = Stats.createStopwatch(InvocationDispatcher.class, "InvokeSend", "Time to send a remote invocation command");

    Stopwatch invokeReceive = Stats.createStopwatch(InvocationDispatcher.class, "InvokeReceive", "Time to receive the result of a remote invocation");
  }
  ...
  public interface InvocationStrategy {
    ...
    public Object dispatchInvocation(InvocationStats stats, Connections pool, InvokeCommand cmd) throws IOException, ClassNotFoundException,
        Throwable;
  }
  ...
  /**
   * Dispatches the given invocation command using the passed in connection pool
   * (which holds connections to a remote server).
   * 
   * @param vmId
   *          the {@link VmId} of the stub that is performing the call.
   * @param pool
   *          {@link Connections} to the server to call.
   * @param cmd
   *          an {@link InvokeCommand} representing the remote method invocation
   *          to be performed.
   * @return the return value of the invocation.
   * @throws java.io.IOException
   *           if an IO problem occurs while performing this operation.
   * @throws ClassNotFoundException
   *           if the class of the invocation's return value could not be found
   *           in the deserialization process.
   * @throws Throwable
   *           if the invocation is colocated and an exception is throws by the
   *           colocated call.
   */
  public Object dispatchInvocation(VmId vmId, Connections pool, InvokeCommand cmd) throws java.io.IOException, ClassNotFoundException, Throwable {
