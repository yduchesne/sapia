...
public class LocalMethod {
  ...
  /**
   * Internally probes its {@link LocalMethodInvoker}s to determine if one can
   * invoke the given method. If one is found, it is returned (otherwise this
   * method returns <code>null</code>).
   * 
   * @param toInvoke
   *          the {@link Method} to invoke.
   * @return the {@link LocalMethodInvoker} that matches the given method, or
   *         <code>null</code> if no matching one was found.
   */
  ...
  /**
   * Keeps {@link LocalMethod} on a per method-name basis, allowing for fast
   * lookup.
   * 
   * @author yduchesne
   * 
  ...
  public static class LocalMethodMap {
     ...
     * @param toInvoke
     *          the {@link Method} to invoke.
     * @return the {@link LocalMethodInvoker} corresponding to the given method,
    ...
    /**
     * @return a default {@link LocalMethodMap}, which is configured to handle
     *         calls to <code>toString()</code>, <code>hashCode()</code>,
     *         <code>equals()</code>.
     */
    public static LocalMethodMap buildDefaultMap() {
      LocalMethodMap map = new LocalMethodMap();
      map.addLocalMethod(new LocalMethod("toString").addLocalMethodInvoker(LocalMethodInvoker.TO_STRING))
