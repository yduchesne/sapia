...
public class PortResourceTest {
  ...
  @Test
  public void testGetPortRangesForAllClusters() {
    String response = resource.getPortRangesForAllClusters(new RequestContext(request, connector));
  ...
  @Test
  public void testGetrangeesForAllClusters2() {
    String response = resource.getPortRangesForAllClusters2(new RequestContext(request, connector));
    JSONArray json = JSONArray.fromObject(response);
    int count = 0;
    for (int i = 0; i < json.size(); i++) {
      JSONObject r = json.getJSONObject(i).getJSONObject("data");
  ...
  public void testgetPortRangesForClusterAndHost() {
    String response = resource.getPortRangesForClusterAndHost(new RequestContext(request, connector));
    JSONArray json = JSONArray.fromObject(response);
    int count = 0;
    for (int i = 0; i < json.size(); i++) {
      JSONObject r = json.getJSONObject(i).getJSONObject("data");
