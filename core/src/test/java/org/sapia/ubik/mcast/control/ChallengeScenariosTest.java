...
public class ChallengeScenariosTest extends EventChannelControllerTestSupport {
  ...
  // testing that the master is properly determine at startup, if no master
  // currently
  // is defined
  @Test
  ...
  // testing that an existing node does not become master if it detects a
  // heartbeat interval timeout and attempts
  ...
  // testing that an existing node does become master if it detects a heartbeat
  // interval timeout and the current
  // master is down
  @Test
