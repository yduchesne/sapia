...
/**
 * The {@link StubProcessor} centralizes logic pertaining to stub creation. This
 * logic can be modified/extended through the implementation of specific
 * strategies (as we'll detail further below).
 * <p>
 * Since Ubik stub are based on dynamic proxies by default, the logic pertaining
 * to stub creation has been sub-divided as "strategies", with each strategy
 * addressing a specific phase in the stub life-cycle:
 * <ol>
 * <li>Invocation handler creation: since dynamic proxies wrap an
 * {@link InvocationHandler}, creating such a handler is the first step in
 * creating a new stub. Ubik expects a specific type of invocation handler (such
 * handlers have to implement the {@link StubInvocationHandler} interface).
 * <li>Stub creation: this step involves actually creating the dynamic proxy
 * that will wrap a given {@link StubInvocationHandler} - created in the first
 * step.
 * <li>Lastly, there is another step, which may or me not be executed: it
 * involves "enriching" a stub that is bound to Ubik's JNDI so that it can
 * perform load-balancing, failover, and discovery of newly bound endpoints -
 * basically, this involves creating so-called "smart stubs" out of basic,
 * plain-vanilla ones.
 * </ol>
 * 
 * Each step in the stub life-cycle is abstracted through an interface.
 * Respectively to the steps given above, these interfaces are as follows:
 * <ol>
 ...
 * </ol>
 * Generally speaking, the {@link StubProcessor} treats strategies along the
 * lines of the chain of responsibility pattern: the processor will query each
 * strategy implementation to inquire about its applicability, using the first
 * one that indeed applies (i.e.: delegating to it the actual operation).
 * <p>
 * Therefore, for any strategy type (invocation handler creation, stub creation,
 * stub enrichment), the default behavior can be overridden by inserting the
 * required strategy implementation (see
 * {@link #insertHandlerCreationStrategy(StubInvocationHandlerCreationStrategy),
 * #insertStubCreationStrategy(StubEnrichmentStrategy),
...
public class StubProcessor implements Module {
  ...
  private Stopwatch stubCreation = Stats.createStopwatch(getClass(), "StubCreation", "Stub creation time");

  private Stopwatch handlerCreation = Stats.createStopwatch(getClass(), "InvocationHandlerCreation", "Handler creation time");

  private Stopwatch enrichment = Stats.createStopwatch(getClass(), "StubEnrichment", "Stub enrichment time");
  ...
  private List<OIDCreationStrategy> oidStrategies = new CopyOnWriteArrayList<OIDCreationStrategy>();

  private List<StubInvocationHandlerCreationStrategy> handlerStrategies = new CopyOnWriteArrayList<StubInvocationHandlerCreationStrategy>();

  private List<StubCreationStrategy> stubStrategies = new CopyOnWriteArrayList<StubCreationStrategy>();

  private List<StubEnrichmentStrategy> enrichmentStrategies = new CopyOnWriteArrayList<StubEnrichmentStrategy>();
  ...
  /**
   * @return the {@link List} of {@link StubInvocationHandlerCreationStrategy}
   *         instances.
   */
  ...
  /**
   * Inserts a given {@link OIDCreationStrategy} at the head of the list of such
   * strategies kept by this instance.
   * 
   * @param strategy
   *          an {@link OIDCreationStrategy}.
   */
  ...
  /**
   * Appends a given {@link OIDCreationStrategy} at the tail of the list of such
   * strategies kept by this instance.
   * 
   * @param strategy
   *          an {@link OIDCreationStrategy}.
  ...
  /**
   * Inserts a given {@link StubInvocationHandlerCreationStrategy} at the head
   * of the list of such strategies kept by this instance.
   * 
   * @param strategy
   *          the {@link StubInvocationHandlerCreationStrategy} to insert.
   */
  ...
  /**
   * Adds a given {@link StubInvocationHandlerCreationStrategy} to the tail of
   * the list of such strategies kept by this instance.
   * 
   * @param strategy
   *          the {@link StubInvocationHandlerCreationStrategy} to add.
   */
  ...
  /**
   * Inserts a given {@link StubCreationStrategy} at the head of the list of
   * such strategies kept by this instance.
   * 
   * @param strategy
   *          the {@link StubCreationStrategy} to insert.
   */
  ...
  /**
   * Adds a given {@link StubCreationStrategy} to the tail of the list of such
   * strategies kept by this instance.
   * 
   * @param strategy
   *          the {@link StubCreationStrategy} to add.
   */
  ...
  /**
   * Inserts a given {@link StubEnrichmentStrategy} at the head of the list of
   * such strategies kept by this instance.
   * 
   * @param strategy
   *          the {@link StubEnrichmentStrategy} to insert.
   */
  ...
  /**
   * Adds a given {@link StubEnrichmentStrategy} to the tail of the list of such
   * strategies kept by this instance.
   * 
   * @param strategy
   *          the {@link StubEnrichmentStrategy} to add.
   */
   ...
   * 
   * @param toExport
   *          an {@link Object} that is meant to be exported as a remote object.
  ...
  /**
   * This method iterates through this instance's
   * {@link StubInvocationHandlerCreationStrategy}, selecting the first one that
   * applies (see {@link StubInvocationHandlerCreationStrategy#apply(Object)}.
   * <p>
   * It delegates to the selected strategy the creation of the
   * {@link StubInvocationHandler} expected by the caller.
   * 
   * @param toExport
   *          the {@link Object} to export (as a remote object).
   * @param context
   *          the {@link RemoteRefContext} that the handler should wrap.
  ...
  /**
   * This method iterates through this instance's {@link StubCreationStrategy},
   * selecting the first one that applies (see
   * {@link StubCreationStrategy#apply(Object)}.
   * <p>
   * It delegates to the selected strategy the creation of the stub that is
   * expected by the caller.
   * 
   * @param toExport
   *          the {@link Object} that was exported (as a remote object).
   * @param context
   *          the {@link StubInvocationHandler} that the stub should wrap.
  ...
  /**
   * This method iterates through this instance's {@link StubEnrichmentStrategy}
   * , selecting the first one that applies (see
   * {@link StubEnrichmentStrategy#apply(Object)}.
   * <p>
   * It delegates to the selected strategy the enrichment of the stub that is
   * passed in.
   * 
   * @param stub
   *          the {@link Object} that corresponds to the stub to enrich.
   * @param bindingInfo
   *          the Ubik JNDI parameters that are used to bind the stub.
