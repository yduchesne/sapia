...
/**
 * This class inherits from {@link BaseFileLogOutput}: it specifies a default log file
 * name, which is set to "ubik-stats". This means that the current log file will be named as follows:
 * 
 * <pre>
 *  ubik-stats.log
 * </pre>
 * 
 * The configuration of this instance can be specified through system properties - see the javadoc
...
public class StatsLogOutput extends BaseFileLogOutput {
  ...
  /**
   * Corresponds to the log file name 
   * (<code>ubik.rmi.stats.log.file.name</code>). Defaults to <code>ubik</code>.
   */
  public static final String PROP_FILE_NAME    = "ubik.rmi.stats.log.file.name";
  /**
   * Corresponds to the maximum file size (in megabytes) before the current
   * log file is archived. 
   * (<code>ubik.rmi.stats.log.file.size</code>). Defaults to 3 megabytes.
   */
   ...
   * (<code>ubik.rmi.stats.log.file.dir</code>). Defaults to the <code>user.dir</code>
   * system property.
   */
  ...
  public StatsLogOutput() {
    super(new Config()
      .setMaxArchive(PROPS.getIntProperty(PROP_MAX_ARCHIVE, Config.DEFAULT_MAX_ARCHIVE))
