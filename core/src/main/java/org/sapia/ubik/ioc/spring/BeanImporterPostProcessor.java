...
/**
 * This bean post processor is used in order to lookup remote objects from
 * remote Ubik JNDI servers. It processes {@link RemoteBeanRef} instances,
 * returning in their place the the actual remote beans that these bean
 * references point to.
 * <p>
 * This post processor supports lazy lookups of services: it allows for Ubik
 * services to appear on the network in an ad-hoc manner. More precisely, given
 * services must not necessarily appear on the network before the applications
 * that depend on them.
 * <p>
 * This allows client applications to startup without risking abrupt termination
 * because of failed service lookups: if a given service is not found, a lazy
 * proxy is created and returned at its place. A discovery mechanism insures
 * that the missing service is eventually retrieved (if it appears on the
 * network).
 * 
 * Note that a {@link NamingServiceBean} must have been configurued beforehand
 * (so that it is used by this post processor when performing lookups).
 * 
 * @see NamingService
 * 
 * @author yduchesne
...
public class BeanImporterPostProcessor implements BeanPostProcessor {
  ...
  public Object postProcessAfterInitialization(Object bean, String name) throws BeansException {
    ...
    if (bean instanceof NamingService) {
      ...
      if (naming == null) {
        throw new IllegalStateException(NamingService.class.getName() + " not passed to this post processor; make sure you declare a "
      ...
      try {
        ...
        Log.warning(getClass(), "Could not resolve remote service: " + ref.getName() + "; attempting discovery");
        ...
        return Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(), ref.getInterfaces(), handler);
      }
    }
    return null;
  }
  ...
  class ServiceDiscoListenerImpl implements ServiceDiscoListener {
    ...
    public void onServiceDiscovered(ServiceDiscoveryEvent anEvent) {
      ...
      if (anEvent.getName().equals(handler.getName())
          || (anEvent.getName().charAt(0) == '/' && anEvent.getName().substring(1).equals(handler.getName()))) {
        try {
