...
/**
 * This provider is used in order to lookup remote objects from remote Ubik JNDI
 * servers.
 * <p>
 * This provider supports lazy lookups of services: it allows for Ubik services
 * to appear on the network in an ad-hoc manner. More precisely, given services
 * must not necessarily appear on the network before the applications that
 * depend on them.
 * <p>
 ...
 * </pre>
 * 
 * Note that a {@link NamingService} must have been bound beforehand (so that it
 * can be injected into this instance).
 * 
 * @see NamingService
...
public class RemoteServiceImporter<T> implements Provider<T> {
   ...
   * 
   * @param type
   *          the {@link Class} of the service to retrieve.
   * @param jndiName
   *          the name under which the expected service is bound into Ubik's
   *          JNDI server(s).
   */
  ...
  public T get() {
    ...
    try {
      ...
      return type
          .cast(Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(), new Class[] { type }, new ProxyInvocationHandler(this)));
    }
  }
  ...
  class ServiceDiscoListenerImpl implements ServiceDiscoListener {
    ...
    public void onServiceDiscovered(ServiceDiscoveryEvent anEvent) {
      ...
      if (anEvent.getName().equals(name) || (anEvent.getName().charAt(0) == '/' && anEvent.getName().substring(1).equals(name))) {
        try {
