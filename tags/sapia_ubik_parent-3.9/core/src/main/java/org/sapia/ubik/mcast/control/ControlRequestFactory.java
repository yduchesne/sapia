...
class ControlRequestFactory {
  ...
  static ControlRequest createChallengeRequest(ControllerContext context) {
    ControlRequest req = new ChallengeRequest(context.getClock(), createId(), context.getNode(), context.getChannelCallback().getAddress(), context
        .getChannelCallback().getNodes());
    return req;
  }
  ...
  static ControlRequest createChallengeRequestCopy(ControllerContext context, ControlRequest request, Set<String> targetedNodes) {
    ControlRequest req = new ChallengeRequest(context.getClock(), request.getRequestId(), context.getNode(), context.getChannelCallback()
        .getAddress(), targetedNodes);
    return req;
  }
