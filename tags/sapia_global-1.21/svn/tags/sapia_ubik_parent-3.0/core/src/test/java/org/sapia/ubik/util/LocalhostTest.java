...
public class LocalhostTest {
  ...
  public void testDoSelectForIpPatternSetNoMatch() throws Exception {
    ...
    InetAddress addr = Localhost.doGetAnyLocalAddress(Collections2.arrayToSet(
        InetAddress.getByName("127.0.0.1"), InetAddress.getByName("192.168.1.1"))
    );
  ...
  public void testDoSelectForIpPatternNotSetWithMultiLocalAddress() throws Exception {
    InetAddress addr = Localhost.doGetAnyLocalAddress(Collections2.arrayToSet(InetAddress.getByName("192.168.1.1"), InetAddress.getByName("192.168.1.2")));
    assertTrue("Expected loopback address", addr.getHostAddress().startsWith("127.0"));
  }    
