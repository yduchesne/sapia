...
MAINCLASS=org.sapia.regis.remote.RegistryServer

REGIS_CLASSPATH=../dist/sapia_regis.jar
for JAR in ../lib/*.jar
do
    # if the directory is empty, then it will return the input string
    # this is stupid, so case for it
    if [ "${JAR}" != "../lib/*.jar" ] ; then
        if [ -z "${REGIS_CLASSPATH}" ] ; then
            REGIS_CLASSPATH=${JAR}
        else
            REGIS_CLASSPATH="${JAR}":${REGIS_CLASSPATH}
        fi
    fi
done
...
if $cygwin; then
    ...
    REGIS_CLASSPATH=`cygpath --path --windows "${REGIS_CLASSPATH}"`
fi

${JAVACMD} -cp ${REGIS_CLASSPATH} ${MAINCLASS} "$@"
