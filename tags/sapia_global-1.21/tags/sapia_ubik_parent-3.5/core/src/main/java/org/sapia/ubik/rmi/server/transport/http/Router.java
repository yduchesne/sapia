...
/**
 * An instance of this class maps context paths to {@link Handler}s. If no {@link Handler} could be 
 * found for a given request path at runtime, it will use a catch-all {@link Handler} (configured with
 * {@link #setCatchAllHandler(Handler)}). The latter's responsibility is to handle the unmatched requests 
 * in a way it sees fit (this allows for implementing a delegation mechanism).
 * <p>
 * The default catch-all handler simply returns an empty response with status code 404 if a request 
...
public class Router implements Handler {
   ...
   * @param handler a {@link Handler} instance.
   */
  ...
  /**
   * @param catchAllHandler the {@link Handler} if the request could not be matched to a {@link Handler} that
   * was added through the {@link #addHandler(String, Handler)} method.
   */
