...
/**
 * An instance of this class holds {@link RemoteRefContext} instances, kept on a per-domain and per-object name
 * basis within a {@link StatelessStubTable}.
 * 
 * @author yduchesne
 *
 */
class Contexts {
  ...
  /**
   * Notified when the list of {@link RemoteRefContext} within a {@link Contexts} instance
   * changes (notification does not occur in the context of removals).
   */
  public interface UpdateListener {
    
    /**
     * @param contexts the {@link Collection} of {@link RemoteRefContext} in the {@link Contexts}
     * instance from which the notification originates.
     */
  ...
  /**
   * @param listener an {@link UpdateListener}.
   */
  ...
  /**
   * @param toAdd the {@link RemoteRefContext}s to add to this instance.
   */
  synchronized void addAll(Collection<RemoteRefContext> toAdd) {
    
    Set<RemoteRefContext> currentSet = Collections2.convertAsSet(
        contexts, 
        new Function<RemoteRefContext, WeakReference<RemoteRefContext>>() {
          @Override
    ...
    contexts = Collections2.convertAsList(
        currentSet, 
        new Function<WeakReference<RemoteRefContext>, RemoteRefContext>() {
          @Override
  ...
  synchronized Collection<RemoteRefContext> getContexts() {
    return Collections2.convertAsList(
        contexts, 
        new Function<RemoteRefContext, WeakReference<RemoteRefContext>>(){
          @Override
          public RemoteRefContext call(WeakReference<RemoteRefContext> ref) {
  ...
  /**
   * @param toRemove the {@link RemoteRefContext} to remove from this instance.
   */
