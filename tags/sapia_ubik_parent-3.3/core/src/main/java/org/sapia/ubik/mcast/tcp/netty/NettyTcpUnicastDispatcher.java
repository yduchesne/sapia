...
public class NettyTcpUnicastDispatcher extends BaseTcpUnicastDispatcher implements NettyMcastConsts {
   ...
   * @param the marshalling buffer size.
   */
  ...
  protected void doStart() {
    ...
    ThreadingConfiguration workerConf = ThreadingConfiguration.newInstance()
        .setCorePoolSize(
            config.getIntProperty(SERVER_WORKER_CORE_THREADS_KEY, DEFAULT_SERVER_WORKER_CORE_THREADS))
        .setMaxPoolSize(
            config.getIntProperty(SERVER_WORKER_MAX_THREADS_KEY, DEFAULT_SERVER_WORKER_MAX_THREADS))
        .setQueueSize(
            config.getIntProperty(SERVER_WORKER_QUEUE_SIZE_KEY, DEFAULT_SERVER_WORKER_QUEUE_SIZE))
        .setKeepAlive(
            Time.createSeconds(config.getLongProperty(SERVER_WORKER_KEEP_ALIVE_KEY, 
                DEFAULT_SERVER_WORKER_KEEP_ALIVE)));
