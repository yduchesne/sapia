...
public class RemoteRefReliableTest {
  ...
  public void testSerializeDeserialize() throws Exception {
    RemoteRefContext context = new RemoteRefContext(new DefaultOID(0), new MinaAddress("localhost", 0));
    RemoteRefReliable ref = new RemoteRefReliable(context, "ubik://localhost/test");

    Object proxy = Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(), TestUtils.getInterfacesFor(java.rmi.Remote.class), ref);
  ...
  public void testIsReliableStub() throws Exception {
    ...
    assertTrue("Stub was not enriched for reliability", Proxy.getInvocationHandler(remoteObject) instanceof RemoteRefReliable);
  }
