...
/**
 * A server-side event that can be intercepted for various purposes, mainly
 * monitoring: it signals that a client GC has synchronized with the server GC.
 * 
...
public class GcEvent implements Event {
   ...
   * 
   * @param originAddr
   *          the {@link ServerAddress} of the client from which the GC event
   *          comes.
   * 
   */
  ...
  /**
   * Returns the address of the host from which the GC synchronization call
   * comes.
   * 
  ...
  /**
   * @return the {@link VmId} of the JVM by which the GC event was triggered.
   */
