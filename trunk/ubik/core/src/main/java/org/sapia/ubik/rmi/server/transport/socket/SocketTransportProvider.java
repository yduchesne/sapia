...
public class SocketTransportProvider implements TransportProvider {
   ...
   * <code>ubik.rmi.transport.socket.max-threads</code> system property.
   */
  public static final String MAX_THREADS = "ubik.rmi.transport.socket.max-threads";

  /**
   * This constant corresponds to the
   * <code>ubik.rmi.transport.socket.client-factory</code> system property (it
   * identifies the {@link ConnectionFactory} to use to create client-side
   * connections).
   */
  ...
  /**
   * This constant corresponds to the
   * <code>ubik.rmi.transport.socket.bind-address</code> system property.
   */
  public static final String BIND_ADDRESS = "ubik.rmi.transport.socket.bind-address";

  /**
   * This constant corresponds to the
   * <code>ubik.rmi.transport.socket.port</code> system property.
   */
  ...
  public synchronized Connections getPoolFor(ServerAddress address) throws RemoteException {
    ...
    if (!poolCleanerStarted) {
      Hub.getModules().getTaskManager().addTask(new TaskContext(getClass().getSimpleName(), DEFAULT_STALE_CLIENT_CONNECTION_TIME), new Task() {
        @Override
  ...
  /**
   * @param port
   *          the port on which to start a new server.
