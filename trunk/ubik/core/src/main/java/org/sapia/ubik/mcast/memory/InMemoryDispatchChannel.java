...
public final class InMemoryDispatchChannel {
  ...
  private Map<MulticastAddress, SoftReference<InMemoryBroadcastDispatcher>> broadcastDispatchers = new ConcurrentHashMap<MulticastAddress, SoftReference<InMemoryBroadcastDispatcher>>();

  private Map<ServerAddress, SoftReference<InMemoryUnicastDispatcher>> unicastDispatchers = new ConcurrentHashMap<ServerAddress, SoftReference<InMemoryUnicastDispatcher>>();

  private ExecutorService asyncEventProcessor = Executors.newFixedThreadPool(3, NamedThreadFactory.createWith("InMemoryBroadcastChannel"));

  private ExecutorService syncEventProcessor = Executors.newFixedThreadPool(3, NamedThreadFactory.createWith("InMemoryBroadcastChannel"));
