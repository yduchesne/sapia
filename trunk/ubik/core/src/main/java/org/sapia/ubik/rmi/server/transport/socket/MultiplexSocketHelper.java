...
public class MultiplexSocketHelper {
   ...
   * 
   * @param aSelector
   *          The stream selector of the connector to create.
   * @return The created socket connector.
   */
  public static MultiplexSocketConnector createSocketConnector(StreamSelector aSelector) {
    SocketServer server = (SocketServer) Hub.getModules().getServerTable().getServerFor(MultiplexSocketTransportProvider.MPLEX_TRANSPORT_TYPE);
