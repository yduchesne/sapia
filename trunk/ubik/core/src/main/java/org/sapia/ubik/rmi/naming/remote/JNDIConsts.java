...
public interface JNDIConsts extends org.sapia.ubik.rmi.Consts {
  ...
  /**
   * Corresponds to the remote event that is dispatched when a JNDI server node
  ...
  /**
   * Corresponds to the remote event that is dispatched when a JNDI server node
   * wishes to notify another node (or other nodes) that it exists.
   */
  public static final String JNDI_SERVER_DISCO = "ubik/rmi/naming/server/disco";

  /**
   * Corresponds to the remote event that is dispatched when a JNDI client node
   * appears on the network.
   */
