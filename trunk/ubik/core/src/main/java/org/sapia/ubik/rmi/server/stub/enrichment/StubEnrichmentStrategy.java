...
/**
 * An instance of this interface is expected to perform the enrichment of a stub
 * that is bound in Ubik's JNDI. That is: at binding time, such a strategy is
 * invoked so that it can perform stub substitution.
 */
public interface StubEnrichmentStrategy extends StubStrategy {

  /**
   * An instance of this class holds the JNDI-related information that is used
   * to bind a stub into Ubik's JNDI.
   * 
   * @author yduchesne
   * 
   */
  public static class JndiBindingInfo {
    ...
    public JndiBindingInfo(String baseUrl, Name name, DomainName domainName, MulticastAddress address) {
  ...
  /**
   * @param stub
   *          the stub that should be enriched.
   * @param info
   *          the {@link JndiBindingInfo} holding the JNDI-related parameters
   *          that will be used to bind the stub into Ubik's JNDI.
   ...
   *           if an error occurs while enriching the stub.
   */
