...
class Contexts {
  ...
  synchronized void addAll(Collection<RemoteRefContext> toAdd) {
    ...
    Set<RemoteRefContext> currentSet = Collects.convertAsSet(contexts, new Func<RemoteRefContext, WeakReference<RemoteRefContext>>() {
      @Override
    ...
    contexts = Collects.convertAsList(currentSet, new Func<WeakReference<RemoteRefContext>, RemoteRefContext>() {
      @Override
  ...
  synchronized Collection<RemoteRefContext> getContexts() {
    return Collects.convertAsList(contexts, new Func<RemoteRefContext, WeakReference<RemoteRefContext>>() {
      @Override
      public RemoteRefContext call(WeakReference<RemoteRefContext> ref) {
        return ref.get();
      }
