...
public class StubProcessorTest {
	...
	public void testCreateInvocationHandlerFor() throws Exception {
		...
		StubInvocationHandlerCreationStrategy stra = new StubInvocationHandlerCreationStrategy() {
			...
			@Override
			public StubInvocationHandler createInvocationHandlerFor(Object toExport,
			    RemoteRefContext context) throws RemoteException {
	...
	public void testCreateStubFor() {
		StubCreationStrategy  stra = mock(StubCreationStrategy.class);
		when(stra.createStubFor(
				any(Object.class), 
				any(StubInvocationHandler.class), 
				any(Class[].class))
		).thenReturn(new TestRemoteIntf(){});
	...
	public void testReliableStubEnrichment() throws Exception {
		...
		StubCreationStrategy  stra = new StubCreationStrategy() {
			...
			public Object createStubFor(Object exported, StubInvocationHandler handler, Class<?>[] stubInterfaces) {
				return Proxy.newProxyInstance(
						...
						new Class[]{Stub.class, TestRemoteIntf.class},
						handler
				);
			}			
			@Override
		...
		JndiBindingInfo   bindingInfo = new JndiBindingInfo(
				"ubik://localhost:1099/", 
				new JndiNameParser(new DefaultNameParser()).parse("test/service"), 
				DomainName.parse("localdomain"), 
				new InMemoryBroadcastDispatcher.InMemoryMulticastAddress("test")
		);
	...
	public void testStatelessStubEnrichment() throws Exception {
		...
		StubCreationStrategy  stra = new StubCreationStrategy() {
			...
			public Object createStubFor(Object exported, StubInvocationHandler handler, Class<?>[] stubInterfaces) {
				return Proxy.newProxyInstance(
						Thread.currentThread().getContextClassLoader(), 
						new Class[]{Stub.class, TestRemoteIntf.class, Stateless.class},
						handler
				);
			}			
			@Override
